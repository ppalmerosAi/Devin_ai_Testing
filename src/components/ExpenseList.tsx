    ...
    <ul className="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200 bg-white">
      {expenses.map((expense) => (
        <li key={expense.id} className="flex items-center gap-3 px-4 py-3">
          <span className="w-16 shrink-0 text-sm text-slate-500">{formatShort(expense.date)}</span>
          <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs text-slate-600">
            {expense.category}
          </span>
          <span className="min-w-0 flex-1 truncate text-slate-700">{expense.note}</span>
          ...
          <button
            ...
            className="rounded-md px-2 text-slate-400 transition hover:bg-red-50 hover:text-red-600"
          >
            ×
          </button>
        </li>
